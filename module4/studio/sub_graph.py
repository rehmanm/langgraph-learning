...
def generate_failure_summary(state):
    ...
    fa_summary = "Poor quality of chroma documentation"
    return {
        "fa_summary": fa_summary,
        "processed_logs":[f"failure_analysis_on_log_{failure["id"]}" for failure in failures]
    }
...
failure_graph = fa_builder.compile()
...
def generate_question_summary(state):
    ...
    return{
        "qs_summary": summary, 
        "processed_logs":[f"question_summarization_on_log_{log['id']}" for log in cleaned_logs]
    }
...
question_graph = qs_builder.compile()
...
entry_builder.add_node("failure_analysis", failure_graph)
entry_builder.add_node("question_summarization", question_graph)
